...
|---|---|---|---|---|---|---|---|
| `AddAttributesToFindings` | `-` | - | `findingArns`, `attributes` | - | `AddAttributesToFindingsResponse` | `AccessDeniedException`, `InternalException`, `InvalidInputException`, `NoSuchEntityException`, `ServiceTemporarilyUnavailableException` | Assigns attributes (key and value pairs) to the findings that are specified by the ARNs of the findings. |
...
| `CreateAssessmentTemplate` | `-` | - | `assessmentTargetArn`, `assessmentTemplateName`, `durationInSeconds`, `rulesPackageArns` | - | `CreateAssessmentTemplateResponse` | `AccessDeniedException`, `InternalException`, `InvalidInputException`, `LimitExceededException`, `NoSuchEntityException`, `ServiceTemporarilyUnavailableException` | Creates an assessment template for the assessment target that is specified by the ARN of the assessment target. If the service-linked role isn’t already registered, this action also creates and registers a service-li ... |
...
| `CreateResourceGroup` | `-` | - | `resourceGroupTags` | - | `CreateResourceGroupResponse` | `AccessDeniedException`, `InternalException`, `InvalidInputException`, `LimitExceededException`, `ServiceTemporarilyUnavailableException` | Creates a resource group using the specified set of tags (key and value pairs) that are used to select the EC2 instances to be included in an Amazon Inspector assessment target. The created resource group is then use ... |
...
| `RemoveAttributesFromFindings` | `-` | - | `findingArns`, `attributeKeys` | - | `RemoveAttributesFromFindingsResponse` | `AccessDeniedException`, `InternalException`, `InvalidInputException`, `NoSuchEntityException`, `ServiceTemporarilyUnavailableException` | Removes entire attributes (key and value pairs) from the findings that are specified by the ARNs of the findings where an attribute with the specified key exists. |
...
|---|---|---|---|
| `AccessDeniedException` | `structure` | message, errorCode, canRetry | You do not have required permissions to access the requested resource. |
| `AgentsAlreadyRunningAssessmentException` | `structure` | message, agents, agentsTruncated, canRetry | You started an assessment run, but one of the instances is already participating in another assessment run. |
| `AssessmentRunInProgressException` | `structure` | message, assessmentRunArns, assessmentRunArnsTruncated, canRetry | You cannot perform a specified action if an assessment run is currently in progress. |
| `InternalException` | `structure` | message, canRetry | Internal server error. |
| `InvalidCrossAccountRoleException` | `structure` | message, errorCode, canRetry | Amazon Inspector cannot assume the cross-account role that it needs to list your EC2 instances during the assessment run. |
| `InvalidInputException` | `structure` | message, errorCode, canRetry | The request was rejected because an invalid or out-of-range value was supplied for an input parameter. |
| `LimitExceededException` | `structure` | message, errorCode, canRetry | The request was rejected because it attempted to create resources beyond the current AWS account limits. The error code describes the limit exceeded. |
| `NoSuchEntityException` | `structure` | message, errorCode, canRetry | The request was rejected because it referenced an entity that does not exist. The error code describes the entity. |
| `PreviewGenerationInProgressException` | `structure` | message | The request is rejected. The specified assessment template is currently generating an exclusions preview. |
| `ServiceTemporarilyUnavailableException` | `structure` | message, canRetry | The serice is temporary unavailable. |
| `UnsupportedFeatureException` | `structure` | message, canRetry | Used by the GetAssessmentReport API. The request was rejected because you tried to generate a report for an assessment run that existed before reporting was ... |
| `AddAttributesToFindingsRequest` | `structure` | findingArns, attributes | - |
...
| `DescribeAssessmentRunsResponse` | `structure` | assessmentRuns, failedItems | - |
| `DescribeAssessmentTargetsRequest` | `structure` | assessmentTargetArns | - |
| `DescribeAssessmentTargetsResponse` | `structure` | assessmentTargets, failedItems | - |
| `DescribeAssessmentTemplatesRequest` | `structure` | assessmentTemplateArns | - |
| `DescribeAssessmentTemplatesResponse` | `structure` | assessmentTemplates, failedItems | - |
| `DescribeCrossAccountAccessRoleResponse` | `structure` | roleArn, valid, registeredAt | - |
| `DescribeExclusionsRequest` | `structure` | exclusionArns, locale | - |
| `DescribeExclusionsResponse` | `structure` | exclusions, failedItems | - |
| `DescribeFindingsRequest` | `structure` | findingArns, locale | - |
| `DescribeFindingsResponse` | `structure` | findings, failedItems | - |
| `DescribeResourceGroupsRequest` | `structure` | resourceGroupArns | - |
| `DescribeResourceGroupsResponse` | `structure` | resourceGroups, failedItems | - |
| `DescribeRulesPackagesRequest` | `structure` | rulesPackageArns, locale | - |
| `DescribeRulesPackagesResponse` | `structure` | rulesPackages, failedItems | - |
| `GetAssessmentReportRequest` | `structure` | assessmentRunArn, reportFileFormat, reportType | - |
| `AccessDeniedErrorCode` | `enum` | ACCESS_DENIED_TO_ASSESSMENT_TARGET, ACCESS_DENIED_TO_ASSESSMENT_TEMPLATE, ACCESS_DENIED_TO_ASSESSMENT_RUN, ACCESS_DENIED_TO_FINDING, ACCESS_DENIED_TO_RESOURCE_GROUP, ACCESS_DENIED_TO_RULES_PACKAGE, ACCESS_DENIED_TO_SNS_TOPIC, ACCESS_DENIED_TO_IAM_ROLE | - |
| `AgentHealth` | `enum` | HEALTHY, UNHEALTHY, UNKNOWN | - |
| `AgentHealthCode` | `enum` | IDLE, RUNNING, SHUTDOWN, UNHEALTHY, THROTTLED, UNKNOWN | - |
| `AssessmentRunNotificationSnsStatusCode` | `enum` | SUCCESS, TOPIC_DOES_NOT_EXIST, ACCESS_DENIED, INTERNAL_ERROR | - |
| `AssessmentRunState` | `enum` | CREATED, START_DATA_COLLECTION_PENDING, START_DATA_COLLECTION_IN_PROGRESS, COLLECTING_DATA, STOP_DATA_COLLECTION_PENDING, DATA_COLLECTED, START_EVALUATING_RULES_PENDING, EVALUATING_RULES, FAILED, ERROR, COMPLETED, COMPLETED_WITH_ERRORS, ... (+1) | - |
| `AssetType` | `enum` | EC2_INSTANCE | - |
| `FailedItemErrorCode` | `enum` | INVALID_ARN, DUPLICATE_ARN, ITEM_DOES_NOT_EXIST, ACCESS_DENIED, LIMIT_EXCEEDED, INTERNAL_ERROR | - |
| `InspectorEvent` | `enum` | ASSESSMENT_RUN_STARTED, ASSESSMENT_RUN_COMPLETED, ASSESSMENT_RUN_STATE_CHANGED, FINDING_REPORTED, OTHER | - |
| `InvalidCrossAccountRoleErrorCode` | `enum` | ROLE_DOES_NOT_EXIST_OR_INVALID_TRUST_RELATIONSHIP, ROLE_DOES_NOT_HAVE_CORRECT_POLICY | - |
| `InvalidInputErrorCode` | `enum` | INVALID_ASSESSMENT_TARGET_ARN, INVALID_ASSESSMENT_TEMPLATE_ARN, INVALID_ASSESSMENT_RUN_ARN, INVALID_FINDING_ARN, INVALID_RESOURCE_GROUP_ARN, INVALID_RULES_PACKAGE_ARN, INVALID_RESOURCE_ARN, INVALID_SNS_TOPIC_ARN, INVALID_IAM_ROLE_ARN, INVALID_ASSESSMENT_TARGET_NAME, INVALID_ASSESSMENT_TARGET_NAME_PATTERN, INVALID_ASSESSMENT_TEMPLATE_NAME, ... (+42) | - |
