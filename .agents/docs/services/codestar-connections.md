...
- From the AWS documentation and model: represent documented AWS CodeStar connections workflows in the local mock. Include service-managed state, documented errors, pagination, and asynchronous job state where the model exposes them.
...
- AWS model slug: `codestar-connections`
- AWS SDK for Rust slug: `codestarconnections`
- Model version: `2019-12-01`
- Model file: `vendor/api-models-aws/models/codestar-connections/service/2019-12-01/codestar-connections-2019-12-01.json`
- SDK ID: `CodeStar connections`
- Endpoint prefix: `codestar-connections`
- ARN namespace: `codestar-connections`
- CloudFormation name: `CodeStarconnections`
- CloudTrail event source: `codestarconnections.amazonaws.com`
...
- https://docs.aws.amazon.com/codebuild/latest/userguide/connections-bitbucket-app.html

Research outcomes:
- CodeStar Connections is the predecessor service name for the same connection model now surfaced as CodeConnections in newer documentation.
- Connections link AWS Developer Tools services to external source providers and are identified by connection ARNs.
- CLI-created GitHub connections are pending until completed through the console/provider authorisation flow.
- Installed provider types use hosts to represent provider infrastructure such as GitHub Enterprise Server.
- CodeBuild and CodePipeline require explicit permission to use a connection ARN.

Parity implications:
- Treat CodeStar Connections and CodeConnections as overlapping API generations with compatible connection, host, tag, and state concepts.
- Preserve pending-provider-authorisation semantics for created connections.
- Validate downstream use of connection ARNs by connection state and IAM permission.
