...
- Operations: `DescribeDomain`, `DescribeDomainAutoTunes`, `DescribeDomainChangeProgress`, `DescribeDomainConfig`, `DescribeDomainHealth`, `DescribeDomainNodes`, `DescribeDomains`, `DescribeDryRunProgress`, `DescribeInboundConnections`, `DescribeInsightDetails`, `DescribeInstanceTypeLimits`, `DescribeOutboundConnections`, `DescribePackages`, `DescribeReservedInstanceOfferings`, `DescribeReservedInstances`, `DescribeVpcEndpoints`
- Traits: `paginated` (6)
- Common required input members in this group: `DomainName`

### List

- Operations: `ListApplications`, `ListDataSources`, `ListDirectQueryDataSources`, `ListDomainMaintenances`, `ListDomainNames`, `ListDomainsForPackage`, `ListInsights`, `ListInstanceTypeDetails`, `ListPackagesForDomain`, `ListScheduledActions`, `ListTags`, `ListVersions`, `ListVpcEndpointAccess`, `ListVpcEndpoints`, `ListVpcEndpointsForDomain`
- Traits: `paginated` (7)
- Common required input members in this group: `DomainName`

### Get

- Operations: `GetApplication`, `GetCapability`, `GetCompatibleVersions`, `GetDataSource`, `GetDefaultApplicationSetting`, `GetDirectQueryDataSource`, `GetDomainMaintenanceStatus`, `GetIndex`, `GetPackageVersionHistory`, `GetUpgradeHistory`, `GetUpgradeStatus`
- Traits: `paginated` (2)
- Common required input members in this group: `DomainName`
...
- Common required input members in this group: `DomainName`, `ConnectionId`
...
- Common required input members in this group: `DomainName`, `DataSourceType`, `PackageID`
...
- Common required input members in this group: `DataSourceType`
...
- Operations: `DeregisterCapability`
- Common required input members in this group: -
...
- Operations: `RollbackServiceSoftwareUpdate`
- Common required input members in this group: -
...
| `AddDataSource` | `POST /2021-01-01/opensearch/domain/{DomainName}/dataSource` | - | `DomainName`, `Name`, `DataSourceType` | - | `AddDataSourceResponse` | `BaseException`, `DependencyFailureException`, `DisabledOperationException`, `InternalException`, `LimitExceededException`, `ResourceNotFoundException`, `ValidationException` | Creates a new direct-query data source to the specified domain. For more information, see Creating Amazon OpenSearch Service data source integrations with Amazon S3 . |
| `AddDirectQueryDataSource` | `POST /2021-01-01/opensearch/directQueryDataSource` | - | `DataSourceName`, `DataSourceType` | - | `AddDirectQueryDataSourceResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `LimitExceededException`, `ResourceNotFoundException`, `ValidationException` | Adds a new data source in Amazon OpenSearch Service so that you can perform direct queries on external data. |
| `AddTags` | `POST /2021-01-01/tags` | - | `ARN`, `TagList` | - | `Unit` | `BaseException`, `InternalException`, `LimitExceededException`, `ValidationException` | Attaches tags to an existing Amazon OpenSearch Service domain, data source, or application. Tags are a set of case-sensitive key-value pairs. A domain, data source, or application can have up to 10 tags. For more inf ... |
| `AssociatePackage` | `POST /2021-01-01/packages/associate/{PackageID}/{DomainName}` | - | `PackageID`, `DomainName` | - | `AssociatePackageResponse` | `AccessDeniedException`, `BaseException`, `ConflictException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Associates a package with an Amazon OpenSearch Service domain. For more information, see Custom packages for Amazon OpenSearch Service . |
| `AssociatePackages` | `POST /2021-01-01/packages/associateMultiple` | - | `PackageList`, `DomainName` | - | `AssociatePackagesResponse` | `BaseException`, `ConflictException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Operation in the Amazon OpenSearch Service API for associating multiple packages with a domain simultaneously. |
...
| `DeregisterCapability` | `DELETE /2021-01-01/opensearch/application/{applicationId}/capability/deregister/{capabilityName}` | - | `applicationId`, `capabilityName` | - | `DeregisterCapabilityResponse` | `AccessDeniedException`, `ConflictException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Deregisters a capability from an OpenSearch UI application. This operation removes the capability and its associated configuration. |
...
| `DescribeInsightDetails` | `POST /2021-01-01/opensearch/insight-details` | - | `Entity`, `InsightId` | - | `DescribeInsightDetailsResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `LimitExceededException`, `ResourceNotFoundException`, `ValidationException` | Describes the details of an existing insight for an Amazon OpenSearch Service domain. Returns detailed fields associated with the specified insight, such as text descriptions and metric data. |
| `DescribeInstanceTypeLimits` | `GET /2021-01-01/opensearch/instanceTypeLimits/{EngineVersion}/{InstanceType}` | - | `InstanceType`, `EngineVersion` | - | `DescribeInstanceTypeLimitsResponse` | `BaseException`, `InternalException`, `InvalidTypeException`, `LimitExceededException`, `ResourceNotFoundException`, `ValidationException` | Describes the instance count, storage, and master node limits for a given OpenSearch or Elasticsearch version and instance type. |
...
| `DissociatePackage` | `POST /2021-01-01/packages/dissociate/{PackageID}/{DomainName}` | - | `PackageID`, `DomainName` | - | `DissociatePackageResponse` | `AccessDeniedException`, `BaseException`, `ConflictException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Removes a package from the specified Amazon OpenSearch Service domain. The package can't be in use with any OpenSearch index for the dissociation to succeed. The package is still available in OpenSearch Service for a ... |
| `DissociatePackages` | `POST /2021-01-01/packages/dissociateMultiple` | - | `PackageList`, `DomainName` | - | `DissociatePackagesResponse` | `BaseException`, `ConflictException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Dissociates multiple packages from a domain simultaneously. |
...
| `GetCapability` | `GET /2021-01-01/opensearch/application/{applicationId}/capability/{capabilityName}` | - | `applicationId`, `capabilityName` | - | `GetCapabilityResponse` | `AccessDeniedException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Retrieves information about a registered capability for an OpenSearch UI application, including its configuration and current status. |
...
| `ListInsights` | `POST /2021-01-01/opensearch/insights` | - | `Entity` | - | `ListInsightsResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `LimitExceededException`, `ResourceNotFoundException`, `ValidationException` | Lists insights for an Amazon OpenSearch Service domain or Amazon Web Services account. Returns a paginated list of insights based on the specified entity, filters, time range, and sort order. |
...
| `PurchaseReservedInstanceOffering` | `POST /2021-01-01/opensearch/purchaseReservedInstanceOffering` | - | `ReservedInstanceOfferingId`, `ReservationName` | - | `PurchaseReservedInstanceOfferingResponse` | `DisabledOperationException`, `InternalException`, `LimitExceededException`, `ResourceAlreadyExistsException`, `ResourceNotFoundException`, `ValidationException` | Allows you to purchase Amazon OpenSearch Service Reserved Instances. |
| `PutDefaultApplicationSetting` | `PUT /2021-01-01/opensearch/defaultApplicationSetting` | - | `applicationArn`, `setAsDefault` | - | `PutDefaultApplicationSettingResponse` | `AccessDeniedException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Sets the default application to the application with the specified ARN. To remove the default application, use the GetDefaultApplicationSetting operation to get the current default and then call the PutDefaultApplica ... |
| `RegisterCapability` | `POST /2021-01-01/opensearch/application/{applicationId}/capability/register` | - | `applicationId`, `capabilityName`, `capabilityConfig` | - | `RegisterCapabilityResponse` | `AccessDeniedException`, `ConflictException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ServiceQuotaExceededException`, `ValidationException` | Registers a capability for an OpenSearch UI application. Use this operation to enable specific capabilities, such as AI features, for a given application. The capability configuration defines the type and settings of ... |
...
| `RollbackServiceSoftwareUpdate` | `POST /2021-01-01/opensearch/serviceSoftwareUpdate/rollback` | - | `DomainName` | - | `RollbackServiceSoftwareUpdateResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Rolls back a service software update for a domain to the previous version. For more information, see Service software updates in Amazon OpenSearch Service . |
| `StartDomainMaintenance` | `POST /2021-01-01/opensearch/domain/{DomainName}/domainMaintenance` | - | `DomainName`, `Action` | - | `StartDomainMaintenanceResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Starts the node maintenance process on the data node. These processes can include a node reboot, an Opensearch or Elasticsearch process restart, or a Dashboard or Kibana restart. |
...
| `UpdateIndex` | `PUT /2021-01-01/opensearch/domain/{DomainName}/index/{IndexName}` | - | `DomainName`, `IndexName`, `IndexSchema` | - | `UpdateIndexResponse` | `AccessDeniedException`, `DependencyFailureException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ThrottlingException`, `ValidationException` | Updates an existing OpenSearch index schema and semantic enrichment configuration. This operation allows modification of field mappings and semantic search settings for text fields. Changes to semantic enrichment con ... |
...
| `UpdatePackageScope` | `POST /2021-01-01/packages/updateScope` | - | `PackageID`, `Operation`, `PackageUserList` | - | `UpdatePackageScopeResponse` | `BaseException`, `DisabledOperationException`, `InternalException`, `ResourceNotFoundException`, `ValidationException` | Updates the scope of a package. Scope of the package defines users who can view and associate a package. |
| `UpdateScheduledAction` | `PUT /2021-01-01/opensearch/domain/{DomainName}/scheduledAction/update` | - | `DomainName`, `ActionID`, `ActionType`, `ScheduleAt` | - | `UpdateScheduledActionResponse` | `BaseException`, `ConflictException`, `InternalException`, `LimitExceededException`, `ResourceNotFoundException`, `SlotNotAvailableException`, `ValidationException` | Reschedules a planned domain configuration change for a later time. This change can be a scheduled service software update or a blue/green Auto-Tune enhancement . |
...
| `ServiceQuotaExceededException` | `structure` | message | An exception for when a request would cause a service quota to be exceeded. |
| `SlotNotAvailableException` | `structure` | SlotSuggestions, message | An exception for attempting to schedule a domain action during an unavailable time slot. |
| `ThrottlingException` | `structure` | message | The request was denied due to request throttling. Reduce the frequency of your requests and try again. |
...
| `AddDataSourceRequest` | `structure` | DomainName, Name, DataSourceType, Description | Container for the parameters to the AddDataSource operation. |
| `AddDataSourceResponse` | `structure` | Message | The result of an AddDataSource operation. |
| `AddDirectQueryDataSourceRequest` | `structure` | DataSourceName, DataSourceType, Description, OpenSearchArns, DataSourceAccessPolicy, TagList | - |
| `AddDirectQueryDataSourceResponse` | `structure` | DataSourceArn | - |
| `AddTagsRequest` | `structure` | ARN, TagList | Container for the parameters to the AddTags operation. Specifies the tags to attach to the domain, data source, or application. |
| `AssociatePackageRequest` | `structure` | PackageID, DomainName, PrerequisitePackageIDList, AssociationConfiguration | Container for the request parameters to the AssociatePackage operation. |
...
| `AssociatePackagesRequest` | `structure` | PackageList, DomainName | - |
| `AssociatePackagesResponse` | `structure` | DomainPackageDetailsList | - |
| `AuthorizeVpcEndpointAccessRequest` | `structure` | DomainName, Account, Service, ServiceOptions | - |
| `AuthorizeVpcEndpointAccessResponse` | `structure` | AuthorizedPrincipal | - |
| `CancelDomainConfigChangeRequest` | `structure` | DomainName, DryRun | - |
| `CancelDomainConfigChangeResponse` | `structure` | CancelledChangeIds, CancelledChangeProperties, DryRun | - |
| `CancelServiceSoftwareUpdateRequest` | `structure` | DomainName | Container for the request parameters to cancel a service software update. |
| `CancelServiceSoftwareUpdateResponse` | `structure` | ServiceSoftwareOptions | Container for the response to a CancelServiceSoftwareUpdate operation. Contains the status of the update. |
| `CreateApplicationRequest` | `structure` | clientToken, name, dataSources, iamIdentityCenterOptions, appConfigs, tagList, kmsKeyArn | - |
| `CreateApplicationResponse` | `structure` | id, name, arn, dataSources, iamIdentityCenterOptions, appConfigs, tagList, createdAt, kmsKeyArn | - |
| `CreateDomainRequest` | `structure` | DomainName, EngineVersion, ClusterConfig, EBSOptions, AccessPolicies, IPAddressType, SnapshotOptions, VPCOptions, CognitoOptions, EncryptionAtRestOptions, NodeToNodeEncryptionOptions, AdvancedOptions, ... (+10) | - |
| `CreateDomainResponse` | `structure` | DomainStatus | The result of a CreateDomain operation. Contains the status of the newly created domain. |
| `CreateIndexRequest` | `structure` | DomainName, IndexName, IndexSchema | - |
| `CreateIndexResponse` | `structure` | Status | - |
| `CreateOutboundConnectionRequest` | `structure` | LocalDomainInfo, RemoteDomainInfo, ConnectionAlias, ConnectionMode, ConnectionProperties | Container for the parameters to the CreateOutboundConnection operation. |
| `CreateOutboundConnectionResponse` | `structure` | LocalDomainInfo, RemoteDomainInfo, ConnectionAlias, ConnectionStatus, ConnectionId, ConnectionMode, ConnectionProperties | The result of a CreateOutboundConnection request. Contains details about the newly created cross-cluster connection. |
| `AWSServicePrincipal` | `enum` | application_opensearchservice_amazonaws_com | - |
| `ActionSeverity` | `enum` | HIGH, MEDIUM, LOW | - |
| `ActionStatus` | `enum` | PENDING_UPDATE, IN_PROGRESS, FAILED, COMPLETED, NOT_ELIGIBLE, ELIGIBLE | - |
| `ActionType` | `enum` | SERVICE_SOFTWARE_UPDATE, JVM_HEAP_SIZE_TUNING, JVM_YOUNG_GEN_TUNING | - |
| `AppConfigType` | `enum` | OpensearchDashboardAdminUsers, OpensearchDashboardAdminGroups | - |
| `ApplicationStatus` | `enum` | CREATING, UPDATING, DELETING, ACTIVE, FAILED | - |
| `AutoTuneDesiredState` | `enum` | ENABLED, DISABLED | The Auto-Tune desired state. Valid values are ENABLED and DISABLED. |
| `AutoTuneState` | `enum` | ENABLED, DISABLED, ENABLE_IN_PROGRESS, DISABLE_IN_PROGRESS, DISABLED_AND_ROLLBACK_SCHEDULED, DISABLED_AND_ROLLBACK_IN_PROGRESS, DISABLED_AND_ROLLBACK_COMPLETE, DISABLED_AND_ROLLBACK_ERROR, ERROR | The Auto-Tune state for the domain. For valid states see Auto-Tune for Amazon OpenSearch Service . |
| `AutoTuneType` | `enum` | SCHEDULED_ACTION | Specifies the Auto-Tune type. Valid value is SCHEDULED_ACTION. |
| `CapabilityFailureReason` | `enum` | KMS_KEY_INSUFFICIENT_PERMISSION | - |
