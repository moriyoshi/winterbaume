...
- Operations: `GetCentralizationRuleForOrganization`, `GetS3TableIntegration`, `GetTelemetryEnrichmentStatus`, `GetTelemetryEvaluationStatus`, `GetTelemetryEvaluationStatusForOrganization`, `GetTelemetryRule`, `GetTelemetryRuleForOrganization`
- Traits: `readonly` (7)
- Common required input members in this group: `RuleIdentifier`

### List

- Operations: `ListCentralizationRulesForOrganization`, `ListResourceTelemetry`, `ListResourceTelemetryForOrganization`, `ListS3TableIntegrations`, `ListTagsForResource`, `ListTelemetryRules`, `ListTelemetryRulesForOrganization`
- Traits: `readonly` (7), `paginated` (6)
- Common required input members in this group: -

### Create

- Operations: `CreateCentralizationRuleForOrganization`, `CreateS3TableIntegration`, `CreateTelemetryRule`, `CreateTelemetryRuleForOrganization`
- Common required input members in this group: `RuleName`, `Rule`

### Delete

- Operations: `DeleteCentralizationRuleForOrganization`, `DeleteS3TableIntegration`, `DeleteTelemetryRule`, `DeleteTelemetryRuleForOrganization`
- Common required input members in this group: `RuleIdentifier`
...
- Operations: `UpdateCentralizationRuleForOrganization`, `UpdateTelemetryRule`, `UpdateTelemetryRuleForOrganization`
- Common required input members in this group: `RuleIdentifier`, `Rule`
...
| `GetTelemetryEvaluationStatusForOrganization` | `POST /GetTelemetryEvaluationStatusForOrganization` | `readonly` | - | - | `GetTelemetryEvaluationStatusForOrganizationOutput` | `AccessDeniedException`, `InternalServerException`, `TooManyRequestsException`, `ValidationException` | This returns the onboarding status of the telemetry configuration feature for the organization. It can only be called by a Management Account of an Amazon Web Services Organization or an assigned Delegated Admin Acco ... |
...
|---|---|---|---|
| `AccessDeniedException` | `structure` | Message, amznErrorType | Indicates you don't have permissions to perform the requested operation. The user or role that is making the request must have at least one IAM permissions ... |
...
| `ServiceQuotaExceededException` | `structure` | Message, ResourceId, ResourceType, ServiceCode, QuotaCode, amznErrorType | The requested operation would exceed the allowed quota for the specified resource type. |
| `TooManyRequestsException` | `structure` | Message | The request throughput limit was exceeded. |
| `ValidationException` | `structure` | Message, Errors | Indicates input validation failed. Check your request parameters and retry the request. |
| `CreateCentralizationRuleForOrganizationInput` | `structure` | RuleName, Rule, Tags | - |
...
| `CreateTelemetryRuleInput` | `structure` | RuleName, Rule, Tags | - |
| `CreateTelemetryRuleOutput` | `structure` | RuleArn | - |
| `CreateTelemetryRuleForOrganizationInput` | `structure` | RuleName, Rule, Tags | - |
...
| `DeleteTelemetryRuleForOrganizationInput` | `structure` | RuleIdentifier | - |
| `GetCentralizationRuleForOrganizationInput` | `structure` | RuleIdentifier | - |
| `GetCentralizationRuleForOrganizationOutput` | `structure` | RuleName, RuleArn, CreatorAccountId, CreatedTimeStamp, CreatedRegion, LastUpdateTimeStamp, RuleHealth, FailureReason, CentralizationRule | - |
| `GetS3TableIntegrationInput` | `structure` | Arn | - |
| `GetS3TableIntegrationOutput` | `structure` | Arn, RoleArn, Status, Encryption, DestinationTableBucketArn, CreatedTimeStamp | - |
| `GetTelemetryEnrichmentStatusOutput` | `structure` | Status, AwsResourceExplorerManagedViewArn | - |
| `GetTelemetryEvaluationStatusOutput` | `structure` | Status, FailureReason, HomeRegion, RegionStatuses | - |
| `GetTelemetryEvaluationStatusForOrganizationOutput` | `structure` | Status, FailureReason, HomeRegion, RegionStatuses | - |
| `GetTelemetryRuleInput` | `structure` | RuleIdentifier | - |
| `GetTelemetryRuleOutput` | `structure` | RuleName, RuleArn, CreatedTimeStamp, LastUpdateTimeStamp, TelemetryRule, HomeRegion, IsReplicated, RegionStatuses | - |
| `GetTelemetryRuleForOrganizationInput` | `structure` | RuleIdentifier | - |
| `GetTelemetryRuleForOrganizationOutput` | `structure` | RuleName, RuleArn, CreatedTimeStamp, LastUpdateTimeStamp, TelemetryRule, HomeRegion, IsReplicated, RegionStatuses | - |
| `ListCentralizationRulesForOrganizationInput` | `structure` | RuleNamePrefix, AllRegions, MaxResults, NextToken | - |
| `ListCentralizationRulesForOrganizationOutput` | `structure` | CentralizationRuleSummaries, NextToken | - |
| `ListResourceTelemetryInput` | `structure` | ResourceIdentifierPrefix, ResourceTypes, TelemetryConfigurationState, ResourceTags, MaxResults, NextToken | - |
| `ListResourceTelemetryOutput` | `structure` | TelemetryConfigurations, NextToken | - |
| `ListResourceTelemetryForOrganizationInput` | `structure` | AccountIdentifiers, ResourceIdentifierPrefix, ResourceTypes, TelemetryConfigurationState, ResourceTags, MaxResults, NextToken | - |
| `ListResourceTelemetryForOrganizationOutput` | `structure` | TelemetryConfigurations, NextToken | - |
| `ListS3TableIntegrationsInput` | `structure` | MaxResults, NextToken | - |
| `ListS3TableIntegrationsOutput` | `structure` | IntegrationSummaries, NextToken | - |
| `ListTagsForResourceInput` | `structure` | ResourceARN | - |
| `Action` | `enum` | ALLOW, BLOCK, COUNT, CAPTCHA, CHALLENGE, EXCLUDED_AS_COUNT | Enumeration of WAF actions that can be matched in filter conditions. |
| `CentralizationFailureReason` | `enum` | TRUSTED_ACCESS_NOT_ENABLED, DESTINATION_ACCOUNT_NOT_IN_ORGANIZATION, INTERNAL_SERVER_ERROR | - |
| `DestinationType` | `enum` | CLOUDWATCH_LOGS | - |
| `EncryptedLogGroupStrategy` | `enum` | ALLOW, SKIP | - |
| `EncryptionConflictResolutionStrategy` | `enum` | ALLOW, SKIP | - |
| `EncryptionStrategy` | `enum` | CUSTOMER_MANAGED, AWS_OWNED | - |
| `FilterBehavior` | `enum` | KEEP, DROP | Enumeration of filter actions: KEEP to include log records, DROP to exclude them. |
| `FilterRequirement` | `enum` | MEETS_ALL, MEETS_ANY | Enumeration of condition matching requirements: MEETS_ALL requires all conditions to match, MEETS_ANY requires at least one. |
| `IntegrationStatus` | `enum` | ACTIVE, DELETING | - |
| `LogType` | `enum` | APPLICATION, USAGE, SECURITY_FINDING, ACCESS, CONNECTION | - |
