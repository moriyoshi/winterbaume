...
- Common required input members in this group: `application`
...
|---|---|---|---|---|---|---|---|
| `AssociateAttributeGroup` | `PUT /applications/{application}/attribute-groups/{attributeGroup}` | - | `application`, `attributeGroup` | - | `AssociateAttributeGroupResponse` | `ConflictException`, `InternalServerException`, `ResourceNotFoundException`, `ServiceQuotaExceededException`, `ValidationException` | Associates an attribute group with an application to augment the application's metadata with the group's attributes. This feature enables applications to be described with user-defined details that are machine-readab ... |
| `AssociateResource` | `PUT /applications/{application}/resources/{resourceType}/{resource}` | - | `application`, `resourceType`, `resource` | - | `AssociateResourceResponse` | `ConflictException`, `InternalServerException`, `ResourceNotFoundException`, `ServiceQuotaExceededException`, `ThrottlingException`, `ValidationException` | Associates a resource with an application. The resource can be specified by its ARN or name. The application can be specified by ARN, ID, or name. Minimum permissions You must have the following permissions to associ ... |
| `CreateApplication` | `POST /applications` | `idempotency-token` | `name`, `clientToken` | `clientToken` | `CreateApplicationResponse` | `ConflictException`, `InternalServerException`, `ServiceQuotaExceededException`, `ThrottlingException`, `ValidationException` | Creates a new application that is the top-level node in a hierarchy of related cloud resource abstractions. |
| `CreateAttributeGroup` | `POST /attribute-groups` | `idempotency-token` | `name`, `attributes`, `clientToken` | `clientToken` | `CreateAttributeGroupResponse` | `ConflictException`, `InternalServerException`, `ServiceQuotaExceededException`, `ValidationException` | Creates a new attribute group as a container for user-defined attributes. This feature enables users to have full control over their cloud application's metadata in a rich machine-readable format to facilitate integr ... |
...
| `GetAssociatedResource` | `GET /applications/{application}/resources/{resourceType}/{resource}` | `idempotent` | `application`, `resourceType`, `resource` | - | `GetAssociatedResourceResponse` | `InternalServerException`, `ResourceNotFoundException`, `ValidationException` | Gets the resource associated with the application. |
...
| `ListAssociatedResources` | `GET /applications/{application}/resources` | `idempotent`, `paginated` | `application` | - | `ListAssociatedResourcesResponse` | `InternalServerException`, `ResourceNotFoundException`, `ValidationException` | Lists all of the resources that are associated with the specified application. Results are paginated. If you share an application, and a consumer account associates a tag query to the application, all of the users wh ... |
...
| `SyncResource` | `POST /sync/{resourceType}/{resource}` | - | `resourceType`, `resource` | - | `SyncResourceResponse` | `ConflictException`, `InternalServerException`, `ResourceNotFoundException`, `ThrottlingException`, `ValidationException` | Syncs the resource with current AppRegistry records. Specifically, the resource’s AppRegistry system tags sync with its associated application. We remove the resource's AppRegistry system tags if it does not associat ... |
| `TagResource` | `POST /tags/{resourceArn}` | - | `resourceArn`, `tags` | - | `TagResourceResponse` | `InternalServerException`, `ResourceNotFoundException`, `ValidationException` | Assigns one or more tags (key-value pairs) to the specified resource. Each tag consists of a key and an optional value. If a tag with the same key is already associated with the resource, this action updates its valu ... |
...
|---|---|---|---|---|
| `GetAssociatedResource` | - | `nextToken -> nextToken`, `resourceTagStatus -> resourceTagStatus`, `maxResults -> maxResults` | - | - |
| `ListApplications` | - | `nextToken -> nextToken`, `maxResults -> maxResults` | - | - |
| `ListAssociatedAttributeGroups` | - | `nextToken -> nextToken`, `maxResults -> maxResults` | - | - |
| `ListAssociatedResources` | - | `nextToken -> nextToken`, `maxResults -> maxResults` | - | - |
| `ListAttributeGroups` | - | `nextToken -> nextToken`, `maxResults -> maxResults` | - | - |
| `ListAttributeGroupsForApplication` | - | `nextToken -> nextToken`, `maxResults -> maxResults` | - | - |
...
| `AssociateResourceRequest` | `structure` | application, resourceType, resource, options | - |
| `AssociateResourceResponse` | `structure` | applicationArn, resourceArn, options | - |
| `CreateApplicationRequest` | `structure` | name, description, tags, clientToken | - |
| `CreateApplicationResponse` | `structure` | application | - |
| `CreateAttributeGroupRequest` | `structure` | name, description, attributes, tags, clientToken | - |
...
| `DisassociateResourceRequest` | `structure` | application, resourceType, resource | - |
| `DisassociateResourceResponse` | `structure` | applicationArn, resourceArn | - |
| `GetApplicationRequest` | `structure` | application | - |
| `GetApplicationResponse` | `structure` | id, arn, name, description, creationTime, lastUpdateTime, associatedResourceCount, tags, integrations, applicationTag | - |
| `GetAssociatedResourceRequest` | `structure` | application, resourceType, resource, nextToken, resourceTagStatus, maxResults | - |
| `GetAssociatedResourceResponse` | `structure` | resource, options, applicationTagResult | - |
| `GetAttributeGroupRequest` | `structure` | attributeGroup | - |
| `GetAttributeGroupResponse` | `structure` | id, arn, name, description, attributes, creationTime, lastUpdateTime, tags, createdBy | - |
| `GetConfigurationResponse` | `structure` | configuration | - |
| `ListApplicationsRequest` | `structure` | nextToken, maxResults | - |
| `ListApplicationsResponse` | `structure` | applications, nextToken | - |
| `ListAssociatedAttributeGroupsRequest` | `structure` | application, nextToken, maxResults | - |
| `ListAssociatedAttributeGroupsResponse` | `structure` | attributeGroups, nextToken | - |
| `ListAssociatedResourcesRequest` | `structure` | application, nextToken, maxResults | - |
| `ListAssociatedResourcesResponse` | `structure` | resources, nextToken | - |
| `ListAttributeGroupsRequest` | `structure` | nextToken, maxResults | - |
| `ListAttributeGroupsResponse` | `structure` | attributeGroups, nextToken | - |
| `ListAttributeGroupsForApplicationRequest` | `structure` | application, nextToken, maxResults | - |
| `ListAttributeGroupsForApplicationResponse` | `structure` | attributeGroupsDetails, nextToken | - |
| `ListTagsForResourceRequest` | `structure` | resourceArn | - |
| `ApplicationTagStatus` | `enum` | IN_PROGRESS, SUCCESS, FAILURE | - |
| `AssociationOption` | `enum` | APPLY_APPLICATION_TAG, SKIP_APPLICATION_TAG | - |
| `ResourceGroupState` | `enum` | CREATING, CREATE_COMPLETE, CREATE_FAILED, UPDATING, UPDATE_COMPLETE, UPDATE_FAILED | - |
| `ResourceItemStatus` | `enum` | SUCCESS, FAILED, IN_PROGRESS, SKIPPED | - |
| `ResourceType` | `enum` | CFN_STACK, RESOURCE_TAG_VALUE | - |
| `SyncAction` | `enum` | START_SYNC, NO_ACTION | - |
