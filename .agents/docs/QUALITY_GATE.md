...
### 5.1 Partial Behaviour Inside an Implemented Operation

A stub defers a whole operation, is tagged, and is excluded from the coverage counts. The harder case is an operation that *is* counted as implemented but under-implements something inside it: a sub-language it only partly parses (`UpdateExpression`, `ConditionExpression`, filter/policy grammars, PartiQL and SQL text), a request field it deserialises and never consults (`ConsistentRead`), or a documented limit it never enforces. Coverage is counted per operation, so none of this appears in `API_COVERAGE.md` — the operation reads as done.

The invariant: **winterbaume may under-implement, but never quietly.** Input the emulator cannot faithfully execute is rejected with the error the real service returns for it, which for validation-class failures is almost always a `400 ValidationException` naming the offending construct. Accepting such input and returning success is the one outcome that is always wrong: the API reports work it did not perform, a test suite written against the emulator passes, and the divergence surfaces only when the same code runs against AWS.

Issue #19 is the reference case. `expr::parse_update_expression` returned `Vec<UpdateAction>` with no error channel and recognised five hard-coded `SET` right-hand-side shapes; every other assignment — including the valid and common `SET p = if_not_exists(p, :zero) + :v` — was dropped on the floor behind a `200 OK` with the item unmodified. The drop was deliberate and documented in the function's doc comment, which is why it survived several rounds of edits to that same function: it read as a settled decision rather than an open defect. Treat a comment saying "silently dropped", "best-effort", or "matching the previous implementation's behaviour" as an unfiled bug, not a decision.

Gate checks:

- [ ] Parsers for AWS sub-languages return `Result`, and their callers map the failure onto the service's validation error. A parser that returns the fragments it recognised and discards the rest is a defect however well the discarding is documented.
- [ ] Sub-language support is tested against the grammar the service accepts, not against the implementation's own branch list. Deriving the test matrix from the code under test hides exactly the inputs that were never considered; for each production AWS admits there is a passing test or an explicit rejection test.
- [ ] Ticking a feature name in a dossier, LTM note, or TODO entry (`if_not_exists` supported) states which *shapes* of it work. A function supported in one syntactic position and dropped in another is a partial implementation, and is recorded as one.
- [ ] Request fields that are deserialised but never read are honoured, rejected when set to a non-default value, or recorded as an intentional divergence in the service dossier. A silently inert field is the same failure as a silently dropped expression.
- [ ] Documented service limits the handler does not enforce (batch and action counts, item and payload sizes, duplicate-key rules) are enforced or filed in `TODO.md` with the AWS error they should raise.
- [ ] Constructs the emulator knowingly does not support fail with a message naming the limitation, so the gap is discoverable from the response instead of only from the source.
- [ ] Search for the tell before signing off a crate; each hit is classified as faithful AWS behaviour, an internal non-API path, or a divergence to fix or file:
  ```bash
  rg -ni 'silently (drop|ignor)|best.effort|unrecognised.*(drop|ignor)|ignored for now' \
    crates/winterbaume-*/src
  ```
  This grep is a backstop, not the check. It finds issue #19's own comment, and on the 2026-08-17 workspace-wide run its other 21 hits were all legitimate — ELB Classic really does ignore deletes of a missing load balancer, the S3 and Glacier `persist_*` helpers write a recovery aid off the API path, and the Terraform converters already emit warnings. What it cannot find is the larger category: a divergence nobody wrote a comment about. `ConsistentRead` is deserialised and never consulted anywhere in `winterbaume-dynamodb` and produces no hit at all. Only reading the operation against its AWS contract finds those.

### 5.2 Dossier Conformance

Every service dossier's `## Official AWS Documentation Research` section ends in `Parity implications`. Those bullets are not commentary — they are source-cited claims about how the real service behaves, written down specifically so they can be implemented. Nothing else in this gate checks them.

That omission is how the DynamoDB follow-ups to issue #19 happened. The dossier already said "Keep transaction state changes atomic and **validate same-item duplication before mutation**", and already recorded that strongly consistent reads are unavailable on global secondary indexes. Neither was implemented, for months, while the research sat correct and unread a directory away. §5.1 covers divergence nobody documented; this covers divergence from what the project had already established. The second is the worse failure, because the research cost was paid and then discarded, and no amount of further documentation fixes it.

A recorded claim is resolved in exactly one of three ways, and silence is not one of them:

1. implemented, with a test that fails if it regresses;
2. filed in `TODO.md`, naming the AWS error or behaviour it should produce;
3. recorded in the dossier as an intentional divergence, with the reason.

Gate checks:

- [ ] Every claim under `Research outcomes` and `Parity implications` for the crate under review maps to one of the three resolutions, and the review states which one per claim. Enumerate them with:
  ```bash
  awk '/^## /{inside = ($0 ~ /Official AWS Documentation Research/)} inside && /^- / && !/^- https?:/' \
    .agents/docs/services/<service>.md
  ```
  The command lists the claims; the mapping is judgement and cannot be automated. An empty diff is not coverage — a claim with no test, no TODO entry, and no divergence note is an open defect however green the crate is.
- [ ] A parameter carrying a sublanguage — expression, filter, policy, SQL, document path — has its grammar recorded in the dossier. Smithy types every one of them as `String`, so no generated artefact can reveal that the parameter has a syntax at all; the grammar has to be transcribed from the service documentation by hand or it exists nowhere. Every DynamoDB defect in the issue-#19 lineage lived inside one of those strings.
- [ ] Behaviour learned from AWS documentation during the work is written back to the dossier with the page cited, so the next reader inherits it instead of re-deriving it.
- [ ] A research outcome found to be wrong is corrected in place, with its source, rather than left standing beside code that contradicts it. Dossiers are evidence, not scripture.
- [ ] When the dossier and the code disagree, the AWS page decides — not the code, and not the dossier. Reading the implementation tells you what it does; only the primary source tells you what it should do, and a suite derived from the implementation cannot tell the difference.
...
- [ ] Behavioural claims in the dossier's `Research outcomes` and `Parity implications` have tests, or a `TODO.md` entry, or a recorded divergence — see §5.2.
