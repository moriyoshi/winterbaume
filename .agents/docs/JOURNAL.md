...
## 2026-05-13: update-readme skill absorbs Terraform coverage; tf_schema module extracted

### Context

The user noticed that `docs/reference/terraform.md` was stale ( still
showed 267 resource types and only a handful of EC2 entries ) even
though many EC2 converters had been added in recent commits. The
`update-readme` skill regenerated API operation coverage and the
service crate READMEs but did not touch the Terraform coverage
doc, so the page drifted independently every time the converter
surface grew.

While investigating, the two Terraform coverage generators
( `generate_terraform_resource_coverage.py` and
`generate_terraform_converter_coverage.py` under the `api-coverage`
skill ) turned out to share a near-identical 40-line
`get_terraform_schema()` helper — fertile ground for drift.

### Findings

- `update-readme/scripts/update_readme.py` hardcoded
  `267 Terraform resource types` in the `docs/reference/services.md`
  footer. Even if the user manually regenerated the Terraform docs,
  this number would never refresh.
- `generate_terraform_resource_coverage.py` answers a different
  question to `generate_terraform_converter_coverage.py`: per-service
  resource-type coverage ( from `crates/winterbaume-terraform/specs/*.toml` )
  versus per-resource attribute coverage ( from regex-parsing the Rust
  converter source ). The two are not redundant; their only real
  overlap is fetching the AWS provider schema.
- The resource-coverage report has slightly more permissive cache
  loading: it accepts either the inner `resource_schemas` dict or the
  full `terraform providers schema -json` envelope. The
  converter-coverage version only accepted the inner form. When
  consolidating, the more permissive behaviour was kept.
- EC2 actually reaches 145 / 139 resource types ( the ">100%" comes
  from the override declaring 37 prefix patterns, some of which match
  resources also classified under other prefixes ). The previous doc
  only listed ~12 EC2 resources because the table had not been
  regenerated since the early days of the converter layer.

### Repository deltas

- `.agents/skills/api-coverage/scripts/tf_schema.py` ( new, 83 lines ):
  shared module exposing `get_terraform_schema(cache_path=None)` and
  `collect_schema_attributes(schema_block)`. Single source of truth
  for the `terraform init && terraform providers schema -json` round
  trip and for the dual-format cache loader.
- `.agents/skills/api-coverage/scripts/generate_terraform_resource_coverage.py`:
  dropped the local `get_terraform_schema()` ( ~45 lines ) and
  unused `json`, `subprocess`, `tempfile` imports; now imports from
  `tf_schema`.
- `.agents/skills/api-coverage/scripts/generate_terraform_converter_coverage.py`:
  same treatment, also dropped unused `os` import. Kept
  `collect_schema_attributes` ( still used at line 254 to enumerate TF
  attributes for each handled resource ).
- `.agents/skills/update-readme/scripts/update_readme.py`: added
  `regenerate_terraform_reports()`, `parse_terraform_resource_report()`,
  `parse_terraform_converter_report()`, `generate_terraform_doc_content()`,
  `update_docs_terraform()`. Threaded a `terraform_resource_count`
  optional through `update_docs_reference()` and
  `generate_docs_reference_content()` so the services.md footer pulls
  the live distinct-resource-type count from the converter report
  ( currently `1,025` ) instead of the hardcoded `267`. Added a
  `--no-terraform` flag for environments without the Terraform CLI.
- `.agents/skills/update-readme/SKILL.md`: documented the new
  Terraform regeneration step and the `--no-terraform` flag.
- `docs/reference/terraform.md`: rewritten by the new generator. Now
  carries the per-service resource-coverage table ( with EC2 at the
  top ), the per-resource attribute coverage summary for all 1025
  distinct types, and the fresh rating distribution
  ( 16 excellent / 728 good / 30 fair / 246 poor / 6 n/a ). The
  previous 267-row table was regenerated by the script the moment it
  ran.
- `docs/reference/services.md`: footer now reads `1,025 Terraform
  resource types …`, populated from the live report rather than the
  hardcoded literal.
- `.agents/docs/TERRAFORM_RESOURCE_COVERAGE.md` and
  `TERRAFORM_CONVERTER_COVERAGE.md`: regenerated as a side-effect of
  the integrated skill run.

### Verification

- `python3 .agents/skills/api-coverage/scripts/generate_terraform_resource_coverage.py --output /tmp/wb_res.md`
  and `generate_terraform_converter_coverage.py --output /tmp/wb_conv.md`
  produced output byte-identical to `.agents/docs/TERRAFORM_*_COVERAGE.md`
  after the refactor ( confirmed via `diff`, exit 0 ). The shared
  module is therefore a behaviour-preserving extraction.
- `python3 .agents/skills/update-readme/scripts/update_readme.py`
  end-to-end ran cleanly: regenerated both Terraform reports, rewrote
  `docs/reference/terraform.md`, updated 224 service crate READMEs,
  updated `docs/reference/services.md` ( with the live `1,025`
  footer ), and regenerated 224 `docs/services/` pages.

### Operational notes

- The Terraform schema fetch ( `terraform init && terraform providers
  schema -json` ) costs ~30 s on cold cache. With the schema cached at
  `.agents-workspace/tmp/tf-schema/aws_provider_schema.json`, both
  reports finish in under 3 s combined. The cache is shared across
  the two scripts via the shared module, so running them back-to-back
  ( as `update-readme` now does ) only pays the fetch cost once.
- The `tf_schema.py` module is colocated with the other scripts under
  `.agents/skills/api-coverage/scripts/`. Importers add
  `Path(__file__).resolve().parent` to `sys.path` rather than turning
  the directory into a package; this matches the convention already
  used in `update_readme.py` for importing `generate_coverage` helpers.
- Line-count delta from the consolidation: 460 + 386 = 846 lines
  before, 398 + 335 + 83 = 816 lines after. The savings are modest
