...
## 2026-07-03 — Added fakecloud as a fifth emulator-comparison column

Added fakecloud ( https://github.com/faiscadev/fakecloud, a Rust AWS emulator ) to the API coverage matrix alongside moto / floci / kumo. It lands as a fifth comparison column across the whole reporting chain and renders as `C` in the per-operation `W[ ] S[ ] M[ ] F[ ] K[ ] C[ ]` markers ( `F` was already floci, so fakecloud took `C` ).

**Data source — the tricky part.** Unlike moto ( `IMPLEMENTATION_COVERAGE.md` ), floci ( `docs/services/*.md` ), and kumo ( clean per-service Go files ), fakecloud does not publish a machine-readable operation list. `conformance-baseline.json` is per-*service* variant counts, not per-operation names, and the README / website parity matrix are aggregate counts only. The usable signal is in the source: every service implements the `AwsService` trait, exposing `fn service_name(&self) -> &str` ( the AWS endpoint prefix ) and `fn supported_actions(&self) -> &[&str]` ( the dispatched operation names ). The parser pairs those two methods per file and resolves named `const <SVC>_ACTIONS: &[&str]` slices. Validation anchor: fakecloud's own README advertises "S3 — 107 ops" and the parser yields exactly 107.

**Key design decisions, each learned the hard way:**
- **Key on `service_name`, not the crate name.** fakecloud hosts several AWS services per crate. `fakecloud-iam` contains both `iam` and `sts`; `fakecloud-cognito` contains `cognito-idp` and `cognito-identity`. Keying on the endpoint prefix keeps each in its own column ( result: iam 176, sts 11 — cleanly split ). `FAKECLOUD_SERVICE_TO_MODEL` maps the endpoint prefix to the model dir ( `monitoring`→cloudwatch, `events`→eventbridge, `states`→sfn, `logs`→cloudwatch-logs, `tagging`→resource-groups-tagging-api, `apigateway`→apigatewayv2, `apigatewayv1`→api-gateway, ... ).
- **Regex traps.** ( 1 ) service-name literals may contain hyphens ( `rds-data`, `cognito-idp` ), so the service-name pattern must allow `-` — the operation-name pattern must not. ( 2 ) Some services use `&'static str` / `&[&'static str]` return types; the signature and const-type patterns must tolerate an optional lifetime. Both bugs manifested as whole services silently missing from the output.
- **No brace-matching over raw Rust.** An early impl-block parser miscounted `{`/`}` inside string/char literals in large `handle()` bodies. Replaced with position-based pairing ( each `service_name` → the next `supported_actions` before the following `service_name` ), since fakecloud keeps each `AwsService` impl in its own file.

**Remote fetch.** fakecloud's operation surface spans ~880 crate source files, so per-file GitHub fetches ( the kumo pattern ) would be far too many requests. `_fetch_fakecloud_remote` downloads the repo tarball in a single request and parses the `crates/fakecloud-*/src/**/*.rs` members in memory. Cache / vendor-clone fallbacks mirror the other emulators; the cache dir is gitignored, so the tarball path is what repopulates a fresh checkout.

**Surfaces touched:** `generate_coverage.py` ( mapping table, parser, tarball fetch, cache, and the fakecloud column throughout the report ), `update_readme.py` ( the column-count-sensitive overview-table and `W/S/M/F/K/C` marker regexes, plus every README / docs render function ), both `SKILL.md` docs, and the regenerated `API_COVERAGE.md` + root / per-crate READMEs + `docs/` pages. Headline result: **fakecloud 3980 / 11367 operations across 224 services ( 35.0% )** — second only to Winterbaume, ahead of moto ( 29.0% ). Coverage maintenance now has five mapping surfaces; see [`LTM/workspace-readmes-and-service-examples.md`](./LTM/workspace-readmes-and-service-examples.md).

## 2026-08-17 — DynamoDB UpdateExpression: rewritten parser, no more silent drops ( issue #19 )

Issue [#19](https://github.com/moriyoshi/winterbaume/issues/19) reported that `UpdateItem` with `SET p = if_not_exists(p, :zero) + :v` — the canonical atomic-counter-with-default idiom — returned `200 OK` and did not modify the item. The report's source-level trace was accurate: `parse_update_expression` returned `Vec<UpdateAction>` with no error channel, and `parse_set_assignment` recognised exactly five hard-coded right-hand-side shapes ( `:v`, `p + :v`, `p - :v`, `if_not_exists(p, :v)`, `list_append(p, :v)` ). The `if_not_exists(` branch required a `)` suffix on the whole right-hand side, so a nested call inside an arithmetic expression fell through every branch to `None` and the assignment was discarded. This was documented behaviour ( "Unrecognised fragments are silently dropped" ), which made every unsupported expression a successful no-op — divergence in the dangerous direction, since the caller's counter never increments and the emulator reports success.

**Fix — a real tokeniser plus recursive-descent parser for the update grammar** ( `crates/winterbaume-dynamodb/src/expr.rs` ).

- `parse_update_expression` now returns `Result<Vec<UpdateAction>, String>`. Both call sites in `handlers.rs` ( `handle_update_item` and the `TransactWriteItems` `Update` branch ) map the message onto a `400 ValidationException`, matching how `ConditionExpression` parse failures were already surfaced. Remedy 1 from the issue, which subsumes remedy 2.
- The clause splitter — which used to `find("SET ")` etc. over an uppercased copy of the string and slice by byte offsets — is replaced by `tokenize_update` + `UpdateParser`. Clause keywords are now real tokens, each clause may appear at most once ( AWS rejects a second `SET` section ), and the comma splitting is grammatical rather than paren-depth counting.
- New `types::SetOperand` holds the right-hand side as an operand tree: `Path`, `Value`, `IfNotExists`, `ListAppend`, `Plus`, `Minus`. The four `UpdateAction::Set*` variants collapse into one `UpdateAction::Set { path, value: SetOperand }`. Nesting is now arbitrary, so `SET p = if_not_exists(p, :zero) + :v` and `SET #l = list_append(if_not_exists(#l, :empty), :extra)` both work, as does `SET c = a - b` between two paths. Exactly one arithmetic operator is permitted per assignment, per the documented `value ::= operand | operand '+' operand | operand '-' operand` production, so a chained `:a + :b + :c` is a syntax error the way it is on AWS. ( Note this is the *UpdateExpression* grammar; the PartiQL `SET` arithmetic recorded under "AWS-Fidelity Rules" in the service dossier is a separate, more permissive surface and is untouched. )
- Undefined `#name` / `:value` placeholders are now errors instead of being silently treated as a literal attribute named `#name`.

**Apply-time evaluation.** `apply_update_actions` also returns `Result<(), String>` and evaluates the operand tree against the item. It works on a clone and commits only on success, so a rejected expression leaves the stored item untouched; `state.rs::update_item` was restructured to build a candidate item and insert it only after the actions apply, so a rejected update on a missing key no longer leaves a key-only item behind. The apply layer now raises the same `ValidationException`s the real service does:

- arithmetic on an attribute that is absent from the item ( "The provided expression refers to an attribute that does not exist in the item" ) — previously treated as an implicit `0`, which is precisely why callers reach for `if_not_exists` in the first place;
- wrong operand types for `+` / `-` / `list_append` ( "Incorrect operand type for operator or function" ) — `list_append` previously tolerated a non-list operand and a missing target attribute;
- `ADD` / `DELETE` against an attribute whose type does not match the delta. The old catch-all fell through to numeric handling, so `ADD` of an `NS` onto an existing `SS` silently overwrote the attribute with `"0"`.

**Numeric precision** ( the issue's fifth additional observation ). `SetArithmetic` and the `ADD` path both went through `f64`. `arith_number` now adds and subtracts through `i128` when both operands are integers — `i128` spans DynamoDB's 38 significant digits — and only falls back to `f64` for fractional operands. `format_number` uses `{:.0}` for integral values below 1e38 instead of the old `< 1e15` guard, so a large integer no longer serialises as `1e16`.

**Known gap made explicit.** List-element dereferences ( `a[0]` ) were never supported: `resolve_path` split on `.` only, so `a[0]` became a segment named `a[0]` that matched nothing and the assignment was dropped. Supporting them needs an indexed path representation ( `Vec<String>` → a segment enum ) across `UpdateAction`, the `get_at_path` / `set_at_path` / `remove_at_path` family, projections, and both backends. Until then the tokeniser rejects `[` with a message naming the limitation, which is at least discoverable. Tracked as `dynamodb-update-expression-list-index`.

**Tests.** 8 new unit tests in `expr.rs` ( nested `if_not_exists` in arithmetic, nested `list_append`, path-to-path arithmetic, exactness past 2^53, all four clauses in one expression, eight rejection cases, undefined placeholders, no-partial-mutation ) and 5 new integration tests ( the issue's exact reproduction repeated three times to prove it increments, `list_append` over `if_not_exists`, `ValidationException` for unparseable expressions plus proof the item was not created, arithmetic on a missing attribute, and 2^53+1 precision ). Suites: 16 unit + 182 integration + 5 scenario, all green. Per-crate clippy + fmt gate clean on `winterbaume-dynamodb`, `winterbaume-dynamodb-redis`, and `winterbaume-dynamodbstreams` ( the latter two match on `DynamoDbError`, which gained a `ValidationError(String)` variant ).

**Deferred.** The issue's other observations are real and unaddressed: `TransactWriteItems` accepts two operations on the same item, ignores the caller's operation order ( puts, then deletes, then updates ), and does not enforce the 100-action limit; `ConsistentRead` is deserialised but never consulted. All four are filed in `TODO.md` rather than folded into this change.

### Addendum — the same-day generalisation into QUALITY_GATE.md §5.1

The issue-#19 post-mortem turned up something worth more than the fix. The dossier already carried the adjacent lesson under `### AWS-Fidelity Rules`: "AWS accepts more SET arithmetic than the docs imply... Preserve those SET RHS paths." Someone learned this exact class of mistake on the PartiQL surface and wrote it down, and the `UpdateExpression` parser in the same crate then made the identical mistake anyway. The note did not transfer because it never named its surface — read while working in `expr.rs` it looks like a fact about the PartiQL parser, a different file. That has been fixed in place ( the rule now says which surface it governs, and the `UpdateExpression` grammar is stated alongside it ).

The conclusion: the fact that would actually have prevented this is not the grammar, it is the error contract — *DynamoDB answers a malformed UpdateExpression with 400 ValidationException, never a silent 200*. One sentence, and it makes the "silently dropped" doc comment read as an obvious defect rather than a settled decision. And that sentence is not DynamoDB-specific. Filed in the dossier it fixes DynamoDB; filed in the gate it applies to every crate.

So `QUALITY_GATE.md` gains **§5.1 Partial Behaviour Inside an Implemented Operation**, sitting under the existing stub section because a stub is the operation-level version of the same idea. §5 says an operation-level deferral is tagged and excluded from the coverage count; §5.1 says the same discipline applies one level down, where an operation counted as implemented under-implements a parameter's sub-language, leaves a deserialised field inert, or skips a documented limit — none of which `API_COVERAGE.md` can see, because coverage is counted per operation. The invariant is stated as: **winterbaume may under-implement, but never quietly.** Gate checks cover sub-language parsers returning `Result`, test matrices derived from the grammar rather than from the implementation's own branch list, feature ticks that name which *shapes* work, inert request fields, unenforced limits, and error messages that name the limitation.

The section also carries a grep for the tell ( "silently dropped", "best-effort" ). It was run workspace-wide before being written down, which is the only reason its limits are known: it catches issue #19's own comment at the pinned commit, and its other 21 hits across the workspace are all legitimate — ELB Classic genuinely ignores deletes of a missing load balancer, the S3 / Glacier `persist_*` helpers write a recovery aid off the API path, the Terraform converters already emit warnings. The important negative result is that it does **not** find `ConsistentRead`, which is deserialised and never consulted and about which nobody wrote a comment. Comment-hunting only finds divergences whose author documented them; the larger category is invisible to grep and needs the operation read against its AWS contract. That caveat is in the gate text so the grep is not mistaken for the check.

### Findings — why this survived, and one adjacent gap found while looking

The fix and the gate change are recorded above. What follows is the post-mortem, kept because the causes are reusable and the last item is a live defect nobody has filed.

**Four causes, compounding.**

1. *The drop was documented, so it read as settled.* The doc comment said unrecognised fragments are silently dropped "matching the previous implementation's behaviour and keeping malformed inputs from breaking the request". Phrased as a considered decision, it survived several rounds of edits to the very function it describes — a reader does not re-litigate what looks already decided. The sentence also contains the actual error: it conflates *malformed* with *unrecognised-by-this-parser*, then assumes malformed input is harmless to accept. Both halves are wrong, since real DynamoDB rejects malformed expressions too.
2. *The tests were derived from the implementation, not from the service.* `test_update_item_set_if_not_exists`, `..._set_list_append`, `..._set_arithmetic` — one test per recognised branch, each added as a regression test at the moment that branch was written. The branch list became the de facto specification. A test matrix generated from the code under test cannot, even in principle, cover the inputs the code never considered.
3. *Every failure mode was a `200`.* No panic, no log line, no failing assertion, and the divergence is observable only against real AWS, which the suite never contacts. The one class of bug with no signal attached is the one that lasts.
4. *Coverage is counted per operation.* `UpdateItem` reads as implemented in `API_COVERAGE.md` and in the dossier. Nothing in the reporting chain measures how much of a parameter's language a parser accepts, so a service can show full coverage with a parser that handles a fraction of one field's grammar.

**On whether a richer dossier would have prevented it.** Partly, but not through the grammar. The dossier already carried the adjacent lesson — "AWS accepts more SET arithmetic than the docs imply" — learned on the PartiQL surface, and the `UpdateExpression` parser in the same crate made the identical mistake anyway, because the note never named which surface it governed. The fact that would actually have worked is one sentence of error contract, not a BNF: *DynamoDB answers a malformed UpdateExpression with `400 ValidationException`, never a silent `200`.* Held next to the doc comment at `expr.rs:672`, that sentence turns a settled decision into an obvious defect. And it is not DynamoDB-specific, which is why it went to `QUALITY_GATE.md` §5.1 rather than into the dossier alone. A second caveat: the dossier is only consulted at tasks big enough to trigger the read-first protocol, and each of the five branches was added as a narrow fix to a narrow report.

**Adjacent gap, verified, not fixed: dotted paths are rejected by the condition/filter parser.** While rewriting the update parser I noticed that `tokenize` ( the condition/filter tokeniser, distinct from the new `tokenize_update` ) has no `.` arm, so a dotted document path falls through to the catch-all. Confirmed by probe against the current tree:

```
info.city = :c                => Err("Unexpected character in expression: '.'")
attribute_exists(info.city)   => Err("Unexpected character in expression: '.'")
attribute_not_exists(a.b)     => Err("Unexpected character in expression: '.'")
parse_projection_expression("info.city") => Some([["info", "city"]])
```
