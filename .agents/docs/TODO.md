...
- [ ] **ec2-terraform-state-layer-gaps**: Close the state/view gaps found during the 100-resource EC2 Terraform converter wave: ~~add `gateway_id` to `RouteTableAssociationView`; add `propagating_vgws` to `RouteTableView`; add `private_dns_enabled` to `VpcEndpointView`; expand `ImageView` with kernel, ramdisk, ENA, SR-IOV, TPM, boot mode, IMDS, image location, and AMI-copy source metadata~~; add a singleton spot datafeed subscription slot; and review whether the new VPC route-server family needs fuller service modelling beyond converter-injected state. **2026-05-17 update**: The first three small state-view additions are done ( commit `6df1cda7` ). The `ImageView` expansion is also done ( same-day follow-up commit ): `types::Image` and `ImageView` gained ten optional fields — `kernel_id`, `ramdisk_id`, `ena_support` ( `Option<bool>` ), `sriov_net_support`, `tpm_support`, `boot_mode`, `imds_support`, `image_location`, `source_image_id`, `source_region` — threaded through both From conversions and the three constructor sites in `state.rs` ( `create_image` for instance-derived AMIs, `register_image` for fresh AMIs, the inline restore-image-from-s3 path which now records `image_location: Some("s3://<bucket>/<key>")` ). `copy_image` was extended to set `source_image_id: Some(<source AMI id>)` on the copy so the AMI-copy lineage is preserved across snapshot/restore. `source_region` is left at the inherited value from the source clone — populating it requires a region parameter at the call site that the current signature does not carry, deferred until a real cross-region copy test demands it. Per-crate gate clean: clippy + fmt --check + test --no-fail-fast all pass on `winterbaume-ec2`. Two larger sub-items remain ( singleton spot datafeed subscription slot, VPC route-server family review ). -- *source: 2026-05-11 (cont.): Close the EC2 Terraform-converter coverage gap; partial 2026-05-17 ( first wave + ImageView )*
