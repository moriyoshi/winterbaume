...
- [ ] **ses-v1-v2-shared-backend**: `winterbaume-ses` and `winterbaume-sesv2` hold completely separate state structs ( `SesV1State` vs `SesState` ) even though real AWS backs both APIs with one backend. Identities, configuration sets, templates, suppression list, dedicated IP pools, and account-level settings created via one API are invisible to the other. Per-family unit of work: identities → configuration sets → templates → suppression list → account-level settings. Receipt rule sets stay v1-only; v2-only families ( contact lists, import/export jobs, multi-region endpoints, tenants, deliverability test reports, reputation entities ) stay where they are. Cross-API integration tests are the right regression guard. **2026-05-18 first family done** ( identities ): `winterbaume-sesv2::SesV2Service::shared_state()` exposes `Arc<BackendState<SesState>>` ; `winterbaume-ses::SesService::with_sesv2_state(arc)` constructor wires the v2 state into v1. `winterbaume-ses` gains a workspace dep on `winterbaume-sesv2`. The 8 v1 identity handlers ( `VerifyEmailAddress`, `VerifyEmailIdentity`, `VerifyDomainIdentity`, `DeleteIdentity`, `ListIdentities`, `ListVerifiedEmailAddresses`, `GetIdentityVerificationAttributes`, `GetIdentityDkimAttributes` ) take `account_id` + `region` now and branch: when wired they read / write `SesState.identities`, when unwired they use the legacy `SesV1State.identities` map. v2 is the canonical store ; v1 reads / writes translate between the wire shapes. v1-specific fields ( DKIM 3-token list, domain verification token ) are derived deterministically from the identity name in the wired path — `dkim1.<domain>` / `dkim2.<domain>` / `dkim3.<domain>` and a hashed verification token that matches `SesV1State::verify_domain_identity`. Cross-API regression test in `crates/winterbaume-ses/tests/integration_test.rs` ( `test_v1_v2_shared_identities_round_trip` ) covers: v1 verify -> v2 lists ; v2 create -> v1 lists ; v1 domain verify -> deterministic verification token + DKIM tokens visible via v1 Get* APIs ; v1 delete -> v2 no longer sees. Per-crate gate clean on both ( ses 32 tests + 1 doctest +2 vs prior 30 ; sesv2 88 tests, no regression ). Remaining sub-families ( configuration sets, templates, suppression list, dedicated IP pools, account-level settings ) follow the same pattern with the same `with_sesv2_state(arc)` constructor — each adds its own group of v1 handlers reading / writing the matching field on `SesState`. -- *source: 2026-05-01 v1/v2 audit; first family closed 2026-05-18*
...
- [x] **sagemakerruntime-endpoint-validation**: Done 2026-05-18. Same control-plane / data-plane sharing pattern as `appconfigdata-shared-state` and `kinesisvideoarchivedmedia-stream-validation`:
  - `winterbaume-sagemaker`: new `SageMakerService::shared_state() -> Arc<BackendState<SageMakerState>>` accessor over the existing `pub(crate) state` field.
  - `winterbaume-sagemakerruntime`: workspace dep on `winterbaume-sagemaker` ; new `SageMakerRuntimeService::with_sagemaker_state(state)` constructor ; `Self::new()` keeps the legacy auto-create-on-first-invocation behaviour for unit-test isolation. When `sagemaker_state` is `Some`, the URL-routed dispatch for `POST /endpoints/{name}/invocations` ( and async / streaming variants ) validates `endpoint_name` against the parent's `endpoints` map ; unknown names return HTTP 400 `ValidationError` with body `"Endpoint <name> of account <account_id> not found"`, matching the real-AWS error message verbatim.
  - The "route the targeted variant through the endpoint-config / model graph so invocation records can capture it" sub-task is **not** implemented here — the runtime state's invocation record does not carry a variant identifier today, and adding one would require a bigger refactor of `SageMakerRuntimeState`. Left as a sub-followup if a real test exercises it.
  - End-to-end integration tests added: ( 1 ) wire both services with shared state, drive `CreateModel` -> `CreateEndpointConfig` -> `CreateEndpoint` on `aws-sdk-sagemaker` then call `InvokeEndpoint` on `aws-sdk-sagemakerruntime` and assert success ; ( 2 ) call `InvokeEndpoint` against an unknown endpoint with both services wired and assert `ValidationError` containing `"not found"` and the endpoint name ; ( 3 ) `Self::new()` ( unwired ) preserves the legacy auto-create path.
  - Per-crate gate clean on both: `winterbaume-sagemaker` clippy + fmt --check + tests pass ; `winterbaume-sagemakerruntime` 23 tests + 1 doctest pass ( +3 vs prior 20 from the new validation tests ). -- *source: 2026-05-01 control-plane / data-plane audit; closed 2026-05-18*
- [x] **kinesisvideoarchivedmedia-stream-validation**: Done 2026-05-18. End-to-end fix across both crates:
  - `winterbaume-kinesisvideo`: new public accessor `KinesisVideoService::shared_state() -> Arc<BackendState<KinesisVideoState>>` exposes the per-account/region state holder, mirroring the `appconfigdata-shared-state` pattern.
  - `winterbaume-kinesisvideoarchivedmedia`: workspace dep on `winterbaume-kinesisvideo` added; dev-dep on `aws-sdk-kinesisvideo` added for the new integration tests. New `KinesisVideoArchivedMediaService::with_kinesisvideo_state(Arc<BackendState<KinesisVideoState>>)` constructor wires the parent state; `Self::new()` keeps the legacy ( unwired, auto-creating ) behaviour. Per-operation handlers ( `GetMediaForFragmentList`, `ListFragments`, `GetHLSStreamingSessionURL`, `GetDASHStreamingSessionURL`, `GetClip`, `GetImages` ) call a new `validate_stream_in_parent` helper after wire-deserialise; when the parent state is wired and the supplied stream name / ARN does not exist in the parent's `streams` map, the request is rejected with `ResourceNotFoundException` ( HTTP 404 ). Resolution accepts both stream-name lookups ( `streams.contains_key` ) and stream-ARN lookups ( linear scan of `streams.values()` for a matching `stream_arn` field ). Fragment payload storage stays in the child crate.
  - URL routing collision fix: both crates' SDKs default to `https://kinesisvideo.<region>.amazonaws.com`, so the archived-media URL pattern gained a second path-anchored entry ( `kinesisvideo\.<region>\.amazonaws\.com/(?:getMediaForFragmentList|listFragments|getHLSStreamingSessionURL|getDASHStreamingSessionURL|getClip|getImages)` ). When registering the two services together in a `MockAws::builder`, the archived-media service must be registered first so its more specific path pattern wins over the parent crate's broader hostname pattern; the doc example in `with_kinesisvideo_state` shows this ordering.
  - Three new integration tests: `test_archived_media_accepts_existing_stream_via_parent_state` ( create a stream via `aws-sdk-kinesisvideo::CreateStream`, then `ListFragments` via archived-media succeeds ); `test_archived_media_rejects_unknown_stream_when_wired` ( asserts `ResourceNotFoundException` when wired and the stream was never created ); `test_archived_media_legacy_auto_create_unchanged_when_unwired` ( `Self::new()` preserves the original auto-create-on-first-request path so legacy unit tests are unaffected ).
  - Per-crate gate clean on both: clippy + fmt --check + test --no-fail-fast all pass ( 62 winterbaume-kinesisvideo tests; 31 winterbaume-kinesisvideoarchivedmedia integration tests + 1 doctest, +3 from the new cross-crate suite ).
  - Same pattern can be applied to future `kinesisvideomedia`, `kinesisvideosignaling`, and `kinesisvideowebrtcstorage` crates when they land. -- *source: 2026-05-01 control-plane / data-plane audit; closed 2026-05-18*
- [ ] **cloudtraildata-channel-validation**: `winterbaume-cloudtraildata` records events with whatever `channel_arn` it is given without checking that the channel exists. Real AWS rejects unknown channel ARNs on `PutAuditEvents` with `ChannelARNInvalidException`. Depend on `winterbaume-cloudtrail` and validate the channel ARN against its `channels` map. **2026-05-18 status**: blocked. `winterbaume-cloudtrail` does not yet implement channel CRUD — `CreateChannel` returns a "not yet implemented" JSON error from `handlers.rs:123` and there is no `channels: HashMap` on `CloudTrailState` or `Channel` shape in `types.rs`. Validation in the child crate would always fail because the parent crate has no channels to look up. Before tackling this entry, first implement `CreateChannel` / `DeleteChannel` / `GetChannel` / `ListChannels` in `winterbaume-cloudtrail` with a real `channels: HashMap<String, Channel>` state field ; then this TODO becomes the standard `with_<parent>_state` data-plane-validation pattern. -- *source: 2026-05-01 control-plane / data-plane audit; status refined 2026-05-18*
...
- [ ] **error-tests-typed-variant-assertion-sweep**: GitHub issue #3 ( HeadBucket 4xx body ) was invisible to our own test suite because the existing `test_head_nonexistent_bucket` only asserted `err.is_err()` — that single assertion fires regardless of whether the SDK resolved `HeadBucketError::NotFound` or fell back to `Unhandled`. The same anti-pattern almost certainly exists across other service crates' error-path tests. Sweep: grep for `expect_err\|is_err()` in `crates/*/tests/`, upgrade each error-path test to assert the typed variant via `err.as_service_error()` ( pattern-match on the typed enum where the operation models named errors, or assert `ErrorMetadata.code` where it does not ). Pair with the saved feedback memory `error-tests-must-assert-typed-variant`. -- *source: 2026-05-23 — S3 HeadBucket diagnosis*
- [ ] **dossier-handler-binding-audit**: For every service crate, cross-check the per-operation `## HTTP Bindings` table in the matching dossier against what the hand-written handler actually plumbs. Members listed under Header / Query / Prefix headers / Payload in the dossier must each be either honoured, intentionally unsupported with a documented rationale, or explicitly ignored with a rationale in the handler. Newly visible after the 2026-05-23 extractor refresh: every restJson1 / restXml service has dozens of optional `@httpHeader` and `@httpQuery` members ( cloudfront 109 rows, api-gateway 39, lambda 13, accessanalyzer 11, etc. ) that the original Operation Detail Matrix's `Required input` column hid. Same anti-pattern as the S3 issue #4 root cause — modelled but optional → dropped on hand-written handlers. Run as a quality-gate addendum per service, not a one-shot fleet sweep. Pair with the saved feedback memory `handler-signatures-drop-modelled-fields`. -- *source: 2026-05-23 — bulk dossier refresh*
- [ ] **dossier-modelled-errors-lower-bound-sweep**: The `errors:` list in any service's Smithy model is a lower bound on the HTTP error codes the operation actually emits. S3's `PutObject` models four errors but documents `412 PreconditionFailed` and `409 ConditionalRequestConflict` for conditional writes. Same pattern likely applies to every service with write or quota-bound surfaces. For each crate where `winterbaume` is implemented at parity, audit the official API Reference's "Errors" / "Response" sections per operation against the modelled `errors:` and add `Behavioural Model Notes` entries for the unmodelled HTTP codes that need to be emitted. Worth scoping per-service rather than fleet-wide. -- *source: 2026-05-23 — S3 PreconditionFailed unmodelled*
...
- [ ] **cloudfront-if-match-enforcement**: CloudFront's Smithy model carries `If-Match` on roughly 50 lifecycle operations -- every `Delete*` / `Update*`, plus `AssociateDistributionTenantWebACL`, `AssociateDistributionWebACL`, `CopyDistribution`, `DisassociateDistributionTenantWebACL`, `DisassociateDistributionWebACL`, `PublishConnectionFunction`, `PublishFunction`, `TestConnectionFunction`, `TestFunction`. Real CloudFront requires the header on most mutations; ignoring it silently overwrites. The full machine-readable list lives in `.agents/docs/services/cloudfront.md` under `## HTTP Bindings` → "Conditional-write/read coverage". Action when these ops are implemented in `winterbaume-cloudfront`: reuse the `check_write_preconditions` pattern from `winterbaume-s3`, return 412 PreconditionFailed with bodyless HEAD-style error response only where the verb is HEAD, and add typed-variant regression tests that exercise both the matching-ETag pass and stale-ETag 412 paths. -- *source: 2026-05-23 -- bulk dossier refresh surfaced CloudFront conditional surface*
- [ ] **cloudfront-keyvaluestore-conditional-headers**: `winterbaume-cloudfront-keyvaluestore` ( if/when implemented at parity ) must enforce `If-Match` and `If-None-Match` on `DeleteKey`, `PutKey`, and `UpdateKeys`. Same pattern as S3: 412 PreconditionFailed on mismatch, no orphan state on rejected writes. See `.agents/docs/services/cloudfront-keyvaluestore.md` HTTP Bindings section. -- *source: 2026-05-23 -- bulk dossier refresh*
- [ ] **iotsitewise-asset-model-conditional-headers**: `winterbaume-iotsitewise` asset-model lifecycle operations -- `CreateAssetModelCompositeModel`, `DeleteAssetModel`, `DeleteAssetModelCompositeModel`, `UpdateAssetModel`, `UpdateAssetModelCompositeModel` -- model RFC 7232 conditional headers; enforce when these ops are implemented. -- *source: 2026-05-23 -- bulk dossier refresh*
...
- [ ] **dossier-important-shapes-empty-marker-refresh**: The 2026-05-23 extractor change renders zero-member shapes as `**empty (no members)**` instead of `-` ( the empty member set on error shapes encodes "the HTTP response carries no body", critical signal for HEAD operations ). The bulk dossier refresh only spliced the new `## HTTP Bindings` section; it deliberately did not regenerate the `## Important Shapes` table. As a result, ~417 dossiers' Important Shapes tables still render `-` for empty error shapes. Refresh approach: a scope-limited script that finds each `## Important Shapes` table row whose Members column is exactly `-` and the shape is in fact zero-member per the Smithy model, then in-place-substitute the new marker. Lower priority than the HTTP Bindings work because the strong-signal use of empty-shape rendering ( HEAD operations ) is already captured narratively in `s3.md`, and other services' HEAD operations are few. -- *source: 2026-05-23 — bulk dossier refresh; cosmetic followup*
- [ ] **dossier-wholesale-refresh-drift**: The bulk dossier refresh on 2026-05-23 only added the new HTTP Bindings section; it did not regenerate any other extractor-emitted section. Some sections may have drifted since their dossier was first generated -- operation counts, error lists, member lists in `Important Shapes`, the `## Operation Detail Matrix` `Errors` column. A wholesale refresh would be valuable but is a real engineering task: ( 1 ) for each dossier, run the extractor and capture the new output, ( 2 ) per top-level section, diff old vs new and only adopt changes that do not clobber hand-edits ( the SKILL.md "high-judgement" sections — `Behavioural Model Notes`, `Possible Usage Scenarios`, `Official AWS Documentation Research`, `Research Checklist for Parity Work`, `Resource Model` — need per-dossier review; the others can be regenerated wholesale ). Best done as a tool ( one Python script that takes a per-section policy: replace vs preserve vs three-way-merge ) rather than ad-hoc. -- *source: 2026-05-23 — bulk dossier refresh*
