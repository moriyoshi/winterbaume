...
- [ ] **invariant-audit-existing-services**: Roll out the cross-call invariant inventory to existing service crates. Suggested sequencing: run `.agents/bin/audit-state-fields.sh --all`, turn real flags into per-crate invariant-audit rows, then use `/write-tests` to backfill inventories and scenario coverage. No silent grandfathering; TODO rows are the audit trail.
  ...
  - **2026-05-17 fleet sweep**: Ran `audit-state-fields.sh --all` ( 224 crates ). Five non-EC2 crates produce shared-counter flags through the new fallback ( raw reports preserved under `.agents-workspace/tmp/audit-<crate>.md` ). Each needs per-crate AWS-doc review to determine real-bug vs defensible-global-counter:
    ...
    - `kinesis`: `self.next_sequence` shared across `put_record`, `put_record_by_arn`, `put_records`. **Likely real divergence from AWS**: real Kinesis sequence numbers are monotonic *per shard*, not globally. The mock's global counter is monotonic globally ( so it's monotonic per shard by accident ) but loses the per-shard reset behaviour. Any consumer that derives shard ownership from sequence-number ranges will misbehave. Recommend splitting into a `next_sequence_per_shard` map keyed by `(stream, shard_id)`.
...
- [ ] **ec2-terraform-state-layer-gaps**: Close the state/view gaps found during the 100-resource EC2 Terraform converter wave: ~~add `gateway_id` to `RouteTableAssociationView`; add `propagating_vgws` to `RouteTableView`; add `private_dns_enabled` to `VpcEndpointView`~~; expand `ImageView` with kernel, ramdisk, ENA, SR-IOV, TPM, boot mode, IMDS, image location, and AMI-copy source metadata; add a singleton spot datafeed subscription slot; and review whether the new VPC route-server family needs fuller service modelling beyond converter-injected state. **2026-05-17 update**: The first three small state-view additions are done. `types::RouteTable` gained `propagating_vgws: Vec<String>`, `types::RouteTableAssociation` gained `gateway_id: Option<String>`, and `types::VpcEndpoint` gained `private_dns_enabled: Option<bool>`; the corresponding view structs, both From conversions ( internal -> view and view -> internal ), and all state.rs constructor sites ( `create_route_table`, `associate_route_table`, `replace_route_table_association`, `create_vpc_endpoint` ) were updated. Per-crate `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, and `cargo test --no-fail-fast` ( 591 main + 13 scenario tests, 0 failures ) all clean. Handlers were intentionally left alone — these fields default to "unset" on every code path, so the wire-level `DescribeRouteTables` / `DescribeVpcEndpoints` responses are byte-identical to before; populating them from the matching request inputs ( `EnableVgwRoutePropagation`, `AssociateRouteTable.GatewayId`, `CreateVpcEndpoint.PrivateDnsEnabled` ) is the natural follow-up but is deferred until a terraform converter or test exercises it. The three larger sub-items ( `ImageView` expansion, spot datafeed subscription slot, VPC route-server family review ) remain open. -- *source: 2026-05-11 (cont.): Close the EC2 Terraform-converter coverage gap; partial 2026-05-17*
