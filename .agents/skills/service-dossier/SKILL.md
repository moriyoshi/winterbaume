...
- **Opaque binary payloads (`@httpPayload` blobs).** For every `@httpPayload` member, resolve its target shape kind. A member whose target is a **blob** is the raw, opaque HTTP body: codegen renders it as `bytes::Bytes` and the deserialiser stores `request.body.clone()` with **no** `std::str::from_utf8` / XML / base64 decoding — UTF-8-validating it `400`s any binary body (issue #12). The dossier must add an "Opaque binary payloads" subsection under Behavioural Model Notes that lists each blob-payload operation and its **true content contract**, because "it's a blob" does not mean the service accepts arbitrary binary. Classify each: *genuinely arbitrary binary* (object stores like S3 / Glacier / MediaStore data, and invoke/publish payloads like Lambda / SageMaker Runtime / IoT `Publish`) vs *format-constrained* (real AWS parses/validates and rejects binary — e.g. API Gateway `Import*` = OpenAPI/CSV, CloudSearch `UploadDocuments` = SDF JSON/XML, IoT `UpdateThingShadow` = JSON, AppConfig feature-flag / JSON-Schema profiles = JSON). Research the required format from AWS docs and note that winterbaume's stub is more permissive than AWS wherever it does not enforce that format. Non-payload blobs stay `String` (base64 on the wire) — do not conflate them.
