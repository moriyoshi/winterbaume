...
impl AwsCognitoUserPoolConverter {
    async fn do_inject(
        ...
        let name = require_str(attrs, "name", "aws_cognito_user_pool")?;
        let pool_id = optional_str(attrs, "id")
        ...
        let mfa_config =
            optional_str(attrs, "mfa_configuration").map(|mfa_configuration| MfaConfigView {
                mfa_configuration,
        ...
        let domain = optional_str(attrs, "domain").map(|domain_str| UserPoolDomainView {
            domain: domain_str,
        ...
        let mut all_custom_attributes = custom_attributes;
        ...
        let deletion_protection =
            optional_str(attrs, "deletion_protection").unwrap_or_else(|| "INACTIVE".to_string());
...
impl AwsCognitoUserPoolClientConverter {
    async fn do_inject(
        ...
        let name = require_str(attrs, "name", "aws_cognito_user_pool_client")?;
        let user_pool_id = require_str(attrs, "user_pool_id", "aws_cognito_user_pool_client")?;
        ...
        let client_id = optional_str(attrs, "id")
        ...
        let client_view = UserPoolClientView {
            ...
            client_secret: optional_str(attrs, "client_secret"),
            explicit_auth_flows,
            allowed_oauth_flows,
            allowed_oauth_scopes,
            callback_urls,
            logout_urls,
            allowed_oauth_flows_user_pool_client: attrs
        ...
        if let Some(pool) = state_view.user_pools.get_mut(user_pool_id) {
            ...
            let mut pool_view = UserPoolView {
                id: user_pool_id.to_string(),
                name: user_pool_id.to_string(),
                arn: format!(
                    "arn:aws:cognito-idp:{}:{}:userpool/{}",
            ...
            state_view
                .user_pools
                .insert(user_pool_id.to_string(), pool_view);
        }
        self.service
